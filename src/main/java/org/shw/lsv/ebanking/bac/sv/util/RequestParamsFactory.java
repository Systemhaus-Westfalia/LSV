...
public final class RequestParamsFactory {
    ...
    public static RequestParams createCamt052Params(MBankAccount bankAccount) {
        ...
        String XMLNS               = bank.get_ValueAsString("XMLNS") + XMLNS_Suffix; //"urn:iso:std:iso:20022:tech:xsd:camt.060.001.05";
    ...
    public static RequestParams createPain001Params_Domestico(MPayment payment) {
        ...
        String PYMT_DOCUMENT_ID      = "PYMT-DOM-" + TestDateUtils.getTodayDate() + "-" + payment.getC_Payment_ID();// Generate a unique ID for testing purposes by appending a random 5-digit number.
    ...
    public static RequestParams createTmst038StatusReportParams(MPayment payment) {
    	String XMLNS_Suffix 			= ".03";
    	MBankAccount bankAccount = (MBankAccount)payment.getC_BankAccount();
    	MBank bank = (MBank)bankAccount.getC_Bank();
        String BIC_SISTEMAS_AEREOS   = bank.get_ValueAsString("BIC_SISTEMAS_AEREOS");
        String BIC_BAC_EL_SALVADOR   = bank.getSwiftCode();
        String PYMT_MESSAGE_ID 		 =  payment.get_ValueAsString("MsgId");
    ...
    public static RequestParams createCamt053Params(Integer pageNumber, MBankAccount bankAccount) {
        ...
        String BIC_SISTEMAS_AEREOS 		= bank.get_ValueAsString("BIC_SISTEMAS_AEREOS"); //"AMERICA3PLX"; // BIC of Company	
        String BIC_BAC_EL_SALVADOR 		= bank.getSwiftCode(); //"BAMCSVSS"; // BIC of BAC EL Salvador
        String ESTADO_CUENTA_MESSAGE_ID = "EdC_" + bankAccount.getAccountNo()   ;
        String MSGDEFIDR           		= bank.get_ValueAsString("MSGDEFIDR");//camt.060.001.05";
    ...
    private static MBPartnerLocation getLocationBill(MBPartner partner) {
