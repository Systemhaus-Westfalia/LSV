...
public class SHW_ImportBankStatement extends SHW_ImportBankStatementAbstract {
	...
	protected String doIt() throws java.lang.Exception {
		...
		commitEx();
		StringBuilder sqlimport = new StringBuilder ("SELECT * FROM I_Bankstatement "
				+ "WHERE I_IsImported='N'").append(clientCheck);
		// gody: 20070113 - Order so the same values are consecutive.
		sqlimport.append(" ORDER BY C_BankAccount_ID, Name, EftStatementDate, EftStatementReference");
		String whereClause = "";
		PreparedStatement pstmt =  null;
		ResultSet rs = null;
		try {

			pstmt = DB.prepareStatement(sqlimport.toString(), get_TrxName());
			rs = pstmt.executeQuery();
			while (rs.next())
			{	
				X_I_BankStatement importStatement = new X_I_BankStatement (getCtx(), rs, get_TrxName());
