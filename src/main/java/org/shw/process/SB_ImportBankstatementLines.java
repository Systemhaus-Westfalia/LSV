...
/** Generated Process for (SB_ImportBankstatementLines)
 *  @author ADempiere (generated) 
 *  @version Release 3.9.4
 */
public class SB_ImportBankstatementLines extends SB_ImportBankstatementLinesAbstract
{
	...
	protected String doIt() throws Exception
	{
		...
		getSelectionKeys().stream().forEach( key -> {
			X_I_BankStatement importStatement = new X_I_BankStatement(getCtx(), key, get_TrxName());
			addToStatement(importStatement);
		});
	...
	private void addToStatement(X_I_BankStatement toImport) {
		...
		AtomicInteger noInsert = new AtomicInteger();
		AtomicInteger lineNo = new AtomicInteger(10);
		AtomicInteger noInsertLine = new AtomicInteger();
	...
	private void prepareImport() throws java.lang.Exception{

		int no = 0;

		StringBuffer sql = null;
		int clientID=Env.getAD_Client_ID(getCtx());		
		String clientCheck = " AND AD_Client_ID= " + clientID;
		//	Set Client, Org, IsActive, Created/Updated
		sql = new StringBuffer ("UPDATE I_BankStatement "
			  + "SET AD_Client_ID = COALESCE (AD_Client_ID,").append (clientID).append ("),"
