 ...
 */
public class SHW_ImportPayment extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		sql = new StringBuffer ("UPDATE I_Payment i "
				  + "SET AD_Org_ID=(SELECT AD_Org_ID FROM AD_Org d WHERE trim(d.value)=trim(i.orgValue)"				 
				  + " AND i.AD_Client_ID=d.AD_Client_ID) "
				  + "WHERE  orgValue IS NOT NULL AND I_IsImported<>'Y'").append (clientCheck);

		no = DB.executeUpdate(sql.toString(), get_TrxName());
		
		sql = new StringBuffer ("UPDATE I_Payment i "
				  + "SET User1_ID=(SELECT c_elementvalue_ID FROM c_elementvalue d WHERE d.value=i.centrodecosto"				 
				  + " AND i.AD_Client_ID=d.AD_Client_ID) "
				  + "WHERE User1_ID IS NULL AND centrodecosto IS NOT NULL AND I_IsImported<>'Y'").append (clientCheck);
		...
		try
		{
			...
			//	payment.setC_ConversionType_ID(imp.getC_ConversionType_ID());
				...
				if (imp.get_ValueAsString("shw_codTransaccion") != null && imp.get_ValueAsString("shw_codTransaccion").length()> 0)
					payment.set_ValueOfColumn("shw_codTransaccion", imp.get_ValueAsString("shw_codTransaccion"));

				//if (imp.get_ValueAsString("") != null && imp.get_ValueAsString("").length()> 0)
				//	payment.set_ValueOfColumn("", imp.get_ValueAsString(""));
